...
{
    public static Logger logger = LoggerFactory.getLogger("org.jaudiotagger.audio.aiff.chunk");
    ...
    {
        AudioFile.logger.trace(loggingName+":Reading chunk");
        if (!isId3v2Tag(chunkData))
        {
            logger.error(loggingName+":Invalid ID3 header for ID3 chunk");
            return false;
        }
        ...
        switch (version)
        {
            case ID3v22Tag.MAJOR_VERSION:
                ...
                AudioFile.logger.trace(loggingName+":Reading ID3V2.2 tag");
                break;
            case ID3v23Tag.MAJOR_VERSION:
                id3Tag = new ID3v23Tag();
                AudioFile.logger.trace(loggingName+":Reading ID3V2.3 tag");
                break;
            case ID3v24Tag.MAJOR_VERSION:
                id3Tag = new ID3v24Tag();
                AudioFile.logger.trace(loggingName+":Reading ID3V2.4 tag");
                break;
            default:
        ...
        {
            AudioFile.logger.error(loggingName+":Exception reading ID3 tag: " + e.getClass().getName() + ": " + e.getMessage());
            return false;
        }
        return true;
    }
