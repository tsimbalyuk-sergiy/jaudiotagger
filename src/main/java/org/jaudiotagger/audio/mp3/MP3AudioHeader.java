...
public class MP3AudioHeader implements AudioHeader
{
    ...
    public boolean seek(final File seekFile, long startByte) throws IOException
    {
        ...
        try
        {
            do
            {
                ...
                if (MPEGFrameHeader.isMPEGFrame(bb))
                {
                    try
                    {
                            MP3AudioHeader.logger.trace("Found Possible header at:" + filePointerCount);
                        ...
                        {
                                MP3AudioHeader.logger.trace("Found Possible XingHeader");
                            try
                            {
                        ...
                        {
                                MP3AudioHeader.logger.trace("Found Possible VbriHeader");
                            try
                            {
        ...
        //Return to start of audio header
            MP3AudioHeader.logger.trace("Return found matching mp3 header starting at" + filePointerCount);
    ...
    {
            MP3AudioHeader.logger.trace("Checking next frame" + seekFile.getName() + ":fpc:" + filePointerCount + "skipping to:" + (filePointerCount + mp3FrameHeader.getFrameLength()));
        ...
        {
            MP3AudioHeader.logger.trace("Frame size is too large to be a frame:" + mp3FrameHeader.getFrameLength());
            return false;
        }
        ...
        {
            MP3AudioHeader.logger.trace("Buffer too small, need to reload, buffer size:" + bb.remaining());
            bb.clear();
            fc.position(filePointerCount);
            fc.read(bb, fc.position());
            bb.flip();
            ...
            {
                //No mp3 exists
                MP3AudioHeader.logger.trace("Nearly at end of file, no header found:");
                return false;
            }
            ...
            {
                //No mp3 exists
                MP3AudioHeader.logger.trace("Nearly at end of file, no room for next frame, no header found:");
                return false;
            }
        }
        ...
        if (MPEGFrameHeader.isMPEGFrame(bb))
        {
            try
            {
                ...
                MP3AudioHeader.logger.trace("Check next frame confirms is an audio header ");
                result = true;
            }
            catch (InvalidAudioFrameException ex)
            {
                MP3AudioHeader.logger.trace("Check next frame has identified this is not an audio header");
                result = false;
            }
        }
        else
        {
            MP3AudioHeader.logger.trace("isMPEGFrame has identified this is not an audio header");
        }
